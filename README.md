...
run takes a few minutes and every run after that is ~90 seconds. The sixteen
figures and the GIF land in `quabbin/output/`; the web-map GeoJSON, the 1893
overlay, and the LiDAR overlays land in `quabbin/map/data/`. (The GIF needs
...
│   └── 13_roads.R         road-connectivity diagram (towns to neighbours)
├── data/
...
│   └── data/              GeoJSON + 1893 & LiDAR overlays (generated by 05–13)
...
- **Interactive map** — Leaflet (vendored locally, no CDN dependency) over Esri
  ...
  study itself produces is served locally.
